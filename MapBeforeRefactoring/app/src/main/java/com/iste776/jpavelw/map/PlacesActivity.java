...
public class PlacesActivity extends AppCompatActivity {
    ...
    private Query<Place> placesQuery;
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(categoryAdapter.getItemCount() > 0){
                    ...
                    //int position = selectedPosition == -1 ? 0 : selectedPosition;
                    //placeForm.putExtra(CATEGORY_ID_KEY, Long.toString(categoryAdapter.getCategory(position).getID()));
                    startActivityForResult(placeForm, REQUEST_CODE);
                } else {
                    addCategory.callOnClick();
                    //Snackbar.make(view, getResources().getString(R.string.empty_category_list_message), Snackbar.LENGTH_LONG).setAction("Action", null).show();
    ...
    private List<Place> filterList(){
        if(this.selectedPosition == -1 && this.categoryAdapter.getItemCount() == 0)
            return new ArrayList<>();
        this.placesQuery = this.placeDao.queryBuilder().where(PlaceDao.Properties.CategoryId.eq(categoryId)).orderAsc(PlaceDao.Properties.Date).build();
        return this.placesQuery.list();
    }
    ...
    private PlaceAdapter.PlaceClickListener placeClickListener = new PlaceAdapter.PlaceClickListener() {
        ...
        public void onPlaceClick(int position) {
            ...
            //Intent geoIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("geo:0,0?q=" + place.toString()));
            geoIntent.setPackage("com.google.android.apps.maps");
            startActivity(geoIntent);
            //Toast.makeText(getApplicationContext(), "ID: " + place.getID() + " Name: " + place.getName() + " Date: " + place.getDate(), Toast.LENGTH_LONG).show();
        }

        @Override
        public void onNavigationClick(int position) {
            Place place = placeAdapter.getPlace(position);
            //Intent geoIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("google.navigation:q=" + place.getLatitude() + "," + place.getLongitude()));
